...
class SqlitePoolStore(AbstractPoolStore):
    ...
    async def get_pending_payment_records(self, count: int) -> List[
        ...
        cursor = await self.connection.execute(
            """
                SELECT
                    launcher_id, puzzle_hash, amount, points, timestamp, is_payment, transaction_id, is_confirmed
                FROM payment
        ...
        ret: List[
            ...
            (
                ...
                uint64(timestamp),
                bool(is_payment),
                bytes32(bytes.fromhex(transaction_id)),
                bool(is_confirmed),
            ) for launcher_id, puzzle_hash, amount, points, timestamp, is_payment, transaction_id, is_confirmed in rows
            if transaction_id
        ]
        return ret
    ...
    async def get_confirming_payment_records(self) -> List[bytes32]:
        cursor = await self.connection.execute(
            "SELECT DISTINCT transaction_id FROM payment WHERE is_payment=1 AND is_confirmed=0 ORDER BY timestamp ASC",
        )
        rows = await cursor.fetchall()
        await cursor.close()
        ret: List[bytes32] = [
            bytes32(bytes.fromhex(transaction_id[0])) for transaction_id in rows if transaction_id[0]
        ]
        return ret
